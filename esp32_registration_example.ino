// ESP32 - Cliente con Registro Automático de Dispositivo y Usuario
// Registra automáticamente el dispositivo y permite asignar usuario
...
bool registerUser(String name, String email = "", String phone = "") {
    HTTPClient http;
    ...
    Serial.println("Registrando usuario...");
    ...
    if (httpResponseCode > 0) {
        ...
        Serial.printf("Error registrando usuario: %d\n", httpResponseCode);
        http.end();
        return false;
    }
}
...
void setup() {
    ...
    if (!checkDeviceRegistration()) {
        // Si no está registrado, registrar usuario y dispositivo
        Serial.println("Iniciando proceso de registro...");

        // Registrar usuario (puedes cambiar estos datos)
        if (registerUser("Juan Pérez", "juan@email.com", "+1234567890")) {
            Serial.println("Usuario registrado exitosamente");

            // Registrar dispositivo
            if (registerDevice("ESP32 Cocina", "Cocina")) {
                Serial.println("Dispositivo registrado exitosamente");
            } else {
                Serial.println("Error registrando dispositivo");
            }
        } else {
            Serial.println("Error registrando usuario");
        }
    } else {
    ...
    Serial.println("ESP32 listo y registrado");
}
